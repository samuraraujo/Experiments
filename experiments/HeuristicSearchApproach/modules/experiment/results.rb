...
def summary()
  ...
  ww "LATEX & #{(100*rr).precision(2).to_s} & #{(100*pc).precision(2).to_s} & #{(100*f1).precision(2).to_s}  & #{XNode.transitions.size} & #{( ($featurecounter.positive_queries +  $featurecounter.negative_queries) / $limit.to_f ).precision(2).to_s}   & #{(Time.now() - $t1).precision(2).to_s}" 
