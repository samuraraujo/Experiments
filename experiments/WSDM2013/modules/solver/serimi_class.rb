...
class Serimi < Solver
  ...
  #####################################################################################
  def class_disambiguation(rdfdata,transitive, datasizewithoutpivot)
    ...
    # ifp =  $textp  + propertyoverflow(rdfdata)
    ifp.uniq!
    # puts ifp
    ...
    # puts "WITHOUT PIVOT"
    # # puts svmmodelbygroup.size - $pivot.size - 1 
    number_homonyms[0..( datasizewithoutpivot -  1 )].each{|v | $number_homonyms = $number_homonyms + v}
    $list_number_homonyms = $list_number_homonyms + number_homonyms[0..(datasizewithoutpivot - 1 )]
  ...
  def solve(data,subjects)
   ...
   #log all candidates
    ...
    svmbygroup = class_disambiguation(data + $pivot, [], data.size)
