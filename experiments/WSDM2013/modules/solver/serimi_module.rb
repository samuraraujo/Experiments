module Serimi_Module
  ...
  def class_queries(solution,data,solver)
    return nil if solution == nil || solution.size == 1
    puts "SOLUTION"
    puts solution
  ...
  def entropy_computation(data)
    ...
    # return discriminability(data)
     ...
     entropy_threshold = [(entropy_threshold.to_f / entropies.size.to_f), 0.2].max
     all_predicates=[]
     sorted_entropies.each{|k,v|
     coverage = (instances.find_all{|s,p| p==k }.uniq.size.to_f/instances_size ) 
     puts k
     puts v
     puts coverage
     all_predicates << k
     predicates << k if v <= entropy_threshold and  coverage > 0.40
     }
    
     puts "ENTROPY THRESHOLD"
     puts entropy_threshold
     entropies.delete_if{|k,v| !predicates.include?(k)}
