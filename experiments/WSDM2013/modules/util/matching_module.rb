...
def bigram_matching(a,b)
  ...
  (jaccard(s1_a.map{|x| x.ngrams(1) }.flatten,s2_a.map{|x| x.ngrams(1)}.flatten) + 2*jaccard(s1_a.map{|x| x.ngrams(2) }.flatten,s2_a.map{|x| x.ngrams(2)}.flatten) + 3*jaccard(s1_a.map{|x| x.ngrams(3) }.flatten,s2_a.map{|x| x.ngrams(3)}.flatten)+ 4*jaccard(s1_a.map{|x| x.ngrams(4) }.flatten,s2_a.map{|x| x.ngrams(4)}.flatten)) / 10.0
...
def advanced_string_matching(a,b)
  ...
  score1 =  essembler(s1,s2,s1_aa,s2_aa)
  ...
  if s1_nsw.size > 0 and s2_nsw.size > 0
    score2 =  essembler(s1_nsw,s2_nsw,s1_a,s2_a)
  weight =   ((s1_aa.size + s2_aa.size) + (s1_a.size + s2_a.size)).to_f / ((s1_aa.size + s2_aa.size) * (s1_a.size + s2_a.size)).to_f
  score2 =(score2 + weight)/2
  # puts score3
...
def essembler(a,b,ta,tb)
  [xmatch_with_expansion(a,b).to_f,jaccard(ta,tb).to_f ].max
end
