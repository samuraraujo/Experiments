...
class XAligner
  ...
  def alignment_algorithm(instances, targetpredicate = ["?p"])
    sourcedata =[]
    targetdata =[]
    tmp2=[]
    ...
    instances[0..limit].each {|s|
      tmp=[]
         Query.new.adapters($session[:origin]).sparql("select ?p ?o where {  #{s.to_s} ?p ?o }").execute.each {|p,o| 
           sourcedata << [s,p,o]
           tmp << [s,p,o] 
           }
       
      tmp2=[]
      targetpredicate.each{|tpre|
      entitylabels.each{|pre|
        t = TransitionQuery.new(AttributeQuery.new(AttributeAlignment.new(pre,tpre),0.6,QueryType::OR),nil).query(s).elements
        
        tmp2 = tmp2 + t       
        targetdata = targetdata + tmp2
      }
      }
      count +=1 if tmp2.size > 0
      groupbysubject << [tmp,tmp2]
    ...
    alignments = refine_alignment(groupbysubject,alignments)
     
    puts "ALIGNEMENTS"
    puts alignments
    setcoverage(alignments, targetdata)
     
    alignments
  end
  def relearngroups( solution, data)
  puts "RELEANRNING GROUUPS"
  
    NaiveBayesClassifier.learning=true
    triples = []
    data.each{|x| triples = triples + x } 

    positives = triples.map {|s,p,o| [s,p,o] if solution.include?(s)  }.compact
    setcoverage( XNode.alignments,positives)
  end
  #group alignment in sets
  def setcoverage(alignments,targetdata) 
     target =  alignments.map{|x| x.target}.uniq
       
     targetdata.delete_if{|s,p,o| !target.include?(p.to_s)}
      
     data = targetdata.map{|s,p,o| [s.to_s,p.to_s]}.uniq
     
     puts "target"
     puts target
     subjects = target.map{|a|data.find_all{|s,p|p==a}.map{|s,p| s} 
     } 
     sets = Hash.new
     subjects.each_index{|x| 
       puts "SS"
       puts  target[x]
        sets = merge(sets,subjects[x],x) 
       } 
     ...
     sets.keys.each{|x|
       ...
       index +=1
        x.each{|idx|
          puts  target[idx]
           alignments.find_all{|t| t.target == target[idx] }.map{|t| t.group = index}
  ...
  #merge two sets when they overlap.
  def merge(a,b,idx)
   ...
   #this method remove alignments that the values do not pass an threshold
  def refine_alignment(groupbysubject, alignments)
    #remove negatives
    remove_alignment=[] 
     alignments.each{|alignment|
      source = alignment.source.to_s
      target = alignment.target.to_s 
      measure=nil  
    groupbysubject.each{|sourcedata,targetdata|
      next if measure == false
     sourcedata=preparedata(sourcedata)
     targetdata=preparedata(targetdata)   
        sourcedata.each{|s,p,o| 
          next if measure == false
          if p.to_s == source  
            # puts p
            # puts source
           targetdata.each{|x,y,z|
              next if measure == false
              if y.to_s == target   
                score = advanced_string_matching(o,z)
                # puts o
                # puts z 
                # puts score
    ...
    puts alignments
    puts "REMOVE"
    puts remove_alignment
    puts "difference"
    puts alignments - remove_alignment
     
    return alignments - remove_alignment
  end
  def preparedata(a)
