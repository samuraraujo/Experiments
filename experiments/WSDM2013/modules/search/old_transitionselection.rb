...
class DecisionTreeSelectionAlgorithm
  ...
  def process(node)
    ...
    @predictors[node.transition.qa.attribute_alignment.group] =  NaiveBayesClassifier.new() if @predictors[node.transition.qa.attribute_alignment.group] == nil
    prediction = true
    # puts "################ PROCESSING #############"
    # puts @previous_reduction
    # puts node
    prediction=@predictors[node.transition.qa.attribute_alignment.group].process(node)
