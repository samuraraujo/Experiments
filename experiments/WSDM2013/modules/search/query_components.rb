...
class TransitionHistory
  ...
  def print
    ...
    ww "TIME: "+@time.to_s
    ww "CARDINALITY: "+@cardinality.to_s
    ww "TOTAL: "+@total.to_s
    ww "SUCCESS: "+ @success.to_s
    ww "FAILURES: " + @failure.to_s
    ww "GLOBAL SUCESS CHANCE: " + probability_success().to_s

    ww "Elapse time: " + (@time/(@total+1)).to_s
    ww "Cardinality Ratio: " + (1 - ((@success).to_f/(@cardinality+1).to_f)).to_s
    ww "Failure Ratio: " + failure_ratio.to_s
    ww "Cost: " + cost.to_s
...
class TransitionQuery
  ...
  def to_s()
    if qc != nil
      qa.attribute_alignment.to_s + " " + qa.threshold.to_s + " " + qtype.to_s + " AND " + qc.attribute.to_s + " " + qc.value.to_s + " " + @weight.to_s
    else
...
class XSearch
  ...
  def update_transitions(solution)
    NaiveBayesClassifier.learning=true
    qc =  XNode.solver.class_queries(solution,@@candidates, OptimalClassSolver.new)
    newtransitions=[]
