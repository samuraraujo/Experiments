...
class Source
  ...
  def set_instances(klass, limit)
    ...
    puts "Elapsed time"
    puts Time.now - t0
    instances = onlyreferenceinstances(instances) if $experiment
    @instances = instances
  end
  #elimites instance that are not in the reference alignment
  def onlyreferenceinstances(instances)
    solution = []
     File.open("#{$current_dir}/../reference/reference.txt", 'r').each { |line|
    solution << line.rstrip
  }
  
  solution.map!{|x| x.split("=")[0]}
   
   
  instances.delete_if{|x| !solution.include?(x.to_s.gsub(">","").gsub("<",""))}
  instances
  end
